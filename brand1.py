...
def get_synced_period():
    ...
    # Pak Games K3 1-Min Base Code
    base_code = "10101" 
    period = now.strftime("%Y%m%d") + base_code + f"{total_minutes:04d}"
    return period

def get_k3_ultra_logic(period):
    """
    K3 EXACT DICE LOGIC:
    Sum range: 3 to 18
    Rules: 3-10 (Small), 11-18 (Big)
    """
    seed = str(period) + "FOZI_K3_ULTRA_WIN_786"
    hash_hex = hashlib.sha256(seed.encode()).hexdigest()
    
    # Dice values (1-6) using different parts of the hash
    d1 = (int(hash_hex[10:12], 16) % 6) + 1
    d2 = (int(hash_hex[20:22], 16) % 6) + 1
    d3 = (int(hash_hex[30:32], 16) % 6) + 1
...
def start_engine():
    ...
    try:
        ...
        if my_key not in res:
            clear()
            print(f"\033[1;31m[!] ACCESS DENIED: APPROVAL REQUIRED")
    ...
    while True:
        try:
            ...
            if current_p != last_p:
                if last_p != "":
                    h_bs, h_eo, h_sum, h_dice = get_k3_ultra_logic(last_p)
                    history_list.insert(0, f"P: {last_p[-4:]} ➔ {h_bs} ({h_sum}) | {h_eo}")
                    if len(history_list) > 8: history_list.pop()
                last_p = current_p

            bs, eo, total_sum, dice = get_k3_ultra_logic(current_p)

            clear()
            print(f"\033[1;32m╔══════════════════════════════════════════════════════════╗")
            print(f"║           👑 {OWNER_NAME} K3 EXPERT 👑           ║")
            print(f"╚══════════════════════════════════════════════════════════╝\033[0m")
            print(f" \033[1;37m[●] SYNC     : \033[1;32mPAK GAMES K3 LIVE")
            print(f" \033[1;37m[●] PERIOD   : \033[1;33m{current_p}")
            print("\033[1;32m" + "━"*58 + "\033[0m")

            print(f"\n   \033[1;33m🎲 K3 VIP PREDICTION (3-18):")
            print(f"   \033[1;37m[EXPECTED] : {bs}")
            print(f"   \033[1;37m[DICE SUM] : \033[1;32m{total_sum}")
            print(f"   \033[1;37m[DICE 123] : \033[1;36m{dice[0]} + {dice[1]} + {dice[2]}")
            print(f"   \033[1;37m[SIDE]     : {eo}")

            print("\033[1;32m\n" + "━"*15 + " [ K3 RECENT LOGS ] " + "━"*23 + "\033[0m")
            if not history_list:
                print("   \033[1;30mWaiting for next draw...")
            else:
            ...
            print(f"\n {color}[⏳] NEXT PREDICTION IN: {rem_s}s \033[0m")
